...
### Pourquoi la sécurité du SE est-elle importante ?
...
### Les logiciels antivirus et les pare-feu ne sont-ils pas suffisants ?
...
### Comment Qubes OS assure-t-il la sécurité ?
...
### Comment Qubes OS se compare-t-il à l’utilisation d’un CD autonome ?
...
### Comment Qubes OS se compare-t-il à l’utilisation de machines virtuelles dans un SE classique ?
...
### Comment Qubes OS se compare-t-il à l’utilisation d’une machine physique séparée ?
...
En bref : il s’agit de solutions irréalistes aujourd’hui. Nous abordons ce sujet en profondeur dans notre [document de spécification d’architecture (en anglais)](/attachment/wiki/QubesArchitecture/arch-spec-0.3.pdf).

### Why does Qubes use virtualization?

Nous croyons qu’il s’agit actuellement de la seule approche viable en pratique pour mettre œuvre une isolation robuste tout en permettant en parallèle une compatibilité avec les applications et les pilotes existants.
...
You can even [manually configure your encryption parameters](/fr/doc/encryption-config/), if you like!

### What do all these terms mean?

Tous les termes propres à Qubes sont définis dans le [glossaire](/fr/doc/glossary).
...
See the [XSA Tracker](/fr/security/xsa/).

### What about this other/new (micro)kernel/hypervisor?

Pour chaque nouvelle discussion au sujet d’un autre (micro)noyau ou hyperviseur en relation avec Qubes, nous vous suggérons fortement d’inclure d’abord des réponses aux questions suivantes :
...
Voici les réponses pour Xen 4.1 (que nous utilisons depuis le 2014-04-28) :
...
Stub domains - Default VMs w/o PCI devices | ND  |
...
Please refer to [this page](/fr/doc/vm-sudo/).

### Why is dom0 so old?

Please see:
- [Installing and updating software in dom0](/fr/doc/software-update-dom0/)
- [Note on dom0 and EOL](/fr/doc/supported-versions/#note-on-dom0-and-eol)
...
Veuillez consulter les [consignes de documentation](/fr/doc/fr/doc-guidelines).

### Will Qubes seek to get certified under the GNU Free System Distribution Guidelines (GNU FSDG)?

Pas actuellement, pour les mêmes raisons que [Debian n'est pas certifié](https://www.gnu.org/distros/common-distros.fr.html).
...
Instead, if you want to obtain your own, trustworthy copy of this website in a secure way, you should clone our [website repo](https://github.com/QubesOS/qubesos.github.io), [verify the PGP signatures on the commits and/or tags](/fr/security/verifying-signatures/#how-to-verify-qubes-repos) (signed by the [doc-signing keys](https://github.com/QubesOS/qubes-secpack/tree/master/keys/doc-signing)), then either [render the site on your local machine](https://github.com/QubesOS/qubesos.github.io/blob/master/README.md#instructions) or simply read the source, the vast majority of which was [intentionally written in Markdown so as to be readable as plain text for this very reason](/fr/doc/doc-guidelines/#markdown-conventions).
...
See the [System Requirements](/fr/doc/system-requirements/).

### Is there a list of hardware that is compatible with Qubes OS?

See the [Hardware Compatibility List](/fr/hcl/).

### Is there any certified hardware for Qubes OS?

See [Certified Hardware](/fr/doc/certified-hardware/).
...
Qubes 4.x requires Intel VT-x with EPT / AMD-V with RVI (SLAT) and Intel VT-d / AMD-Vi (aka AMD IOMMU) for proper functionality (see the [4.x System Requirements](/fr/doc/system-requirements/#qubes-release-4x)). If you are receiving an error message on install saying your "hardware lacks the features required to proceed", check to make sure the virtualization options are enabled in your BIOS/UEFI configuration. You may be able to install without the required CPU features for testing purposes only, but VMs (in particular, sys-net) may not function correctly and there will be no security isolation. For more information, see our post on [updated requirements for Qubes-certified hardware](/news/2016/07/21/new-hw-certification-for-q4/).
...
Certains utilisateurs ont réussi à le faire, mais ce n’est ni recommandé ni pris en charge. Qubes doit être installé à même la machine, « à nu ». (Après tout, il utilise son propre hyperviseur « à nu » !)
...
Vous avez peut-être une carte (avec ou sans fil) qui n’est pas compatible avec les pilotes ouverts livrés avec Qubes. Il existe peut-être des pilotes propriétaires dans le paquet linux-firmware.
...
But if you understand the risk and accept it, read [documentation on multibooting](/fr/doc/multiboot/), 
it begins with an explanation of the risks with such a setup.

### Which version of Qubes am I running?

See [here](/fr/doc/version-scheme/#check-installed-version).
...
Ce problème peut généralement être réglé en effectuant la mise à jour à partir de la ligne de commande.

Dans dom0, ouvrez un terminal et exécutez `sudo qubes-dom0-update`.
...
Please review the notes in the `qvm-prefs` man page and [here](/fr/doc/assigning-devices/) and be aware of the potential risks.
...
See also [here](/fr/doc/assigning-devices/).
...
Si vous souhaitez que Flash ne soit proposé que dans un qube :
...
2. [Enable the appropriate RPMFusion repos in the desired Fedora TemplateVM.](/fr/doc/software-update-domu/#rpmfusion-for-fedora-templatevms)
...
Although external media such as external hard drives or flash drives plugged in via USB are available in the USB qube, it is not recommended to access them directly from inside the USB qube. See [Block (Storage) Devices](/fr/doc/block-devices/) for more information.
...
Un disque entièrement chiffré n’apparaît pas dans Nautilus.
...
Le disque déchiffré est maintenant proposé sur `/mnt`. Après utilisation, démonter et fermer le disque.
...
The problem is that Qubes has [tens of thousands of different users](/fr/statistics/) with radically different needs and purposes.
...
Yes.
In general, the Qubes developers will not use a piece of software unless there is an *easy* way to verify both its **integrity** and **authenticity**, preferably via PGP signatures (see [Verifying Signatures](/fr/security/verifying-signatures/)).
Specifically:
 ...
 * If the software is security-sensitive and requires communication with the outside world, a "split" implementation is highly preferred (for examples, see [Split GPG](/fr/doc/split-gpg/) and [Split Bitcoin](/fr/doc/split-bitcoin/)).
 * If the software has dependencies, these should be packaged and available in repos for a [current, Qubes-supported version](/fr/doc/supported-versions/#templatevms) of Fedora (preferred) or Debian (unless all the insecure dependencies can run in an untrusted VM in a "split" implementation).
...
See [these instructions](/fr/doc/qubes-builder/).

### How do I submit a patch?

See the [Qubes Source Code Repositories](/fr/doc/source-code/) article.
...
Intel TXT (used in [Anti Evil Maid](/fr/doc/anti-evil-maid/)) at least tries to avoid or limit trust in BIOS.
...
Yes, Qubes natively supports automation via [Salt (SaltStack)](/fr/doc/salt/).
There is also the unofficial [ansible-qubes toolkit](https://github.com/Rudd-O/ansible-qubes).
(**Warning:** Since this is an external project that has not been reviewed or endorsed by the Qubes team, [allowing it to manage dom0 may be a security risk](/fr/doc/security-guidelines/#dom0-precautions).)
