...
De hecho /etc/dbus-1/system.d/org.freedesktop.NetworkManager.conf no permite a un usuario estándar ejecutar clientes del administrador de red. Para permitir esto, se ha de cambiar dentro de \<policy context="default"\>:
...
## Error al compilar el gui-agent-linux con error de pulsecore
...
Este error es a causa de que Archlinux actualizó el paquete demasiado rápido. Probablemente se ha publicado una nueva versión de pulseaudio, pero el equipo de qubes aún no ha importado las cabeceras del nuevo desarrollo.

Puede crear nuevas cabeceras falsas simplemente copiando las cabeceras antiguas:
```
...
Puede comprobar la versión actual del pulseaudio de archlinux de este modo:

`sudo chroot chroot-archlinux/ pacman -Qi pulseaudio`

## chroot-archlinux/dev/pts no ha sido desmontado

Este es un problema conocido cuando hay errores durante la compilación. Compruebe lo que está montado usando el comando mount (sin parámetros). Simplemente desmonte lo que pueda (o reinicie su vm si es demasiado vago :) )

# Known problems in building with Qubes R2-B2

## xen-vmm-vm fail to build with a PARSETUPLE related error (FIXED)

Descomentar "\#define HAVE\_ATTRIBUTE\_FORMAT\_PARSETUPLE" en chroot\_archlinux/usr/include/python2.7/pyconfig.h corrige el problema, pero no es la solución adecuada [1]...

Está planeada una corrección mejor para la siguiente versión de python (se publicará cuando se considere solucionado el fallo), y se actualizará en el chroot de archlinux tan pronto como esté disponible.

[1] [https://bugs.python.org/issue17547](https://bugs.python.org/issue17547)

## El proceso de arranque falla sin errores visibles en los registros, pero produce una consola de recuperación
...
## Qubes-OS ahora está usando nombres de variables de xenstore diferentes, lo que hace que la VM archlinux falle al arrancar

Aplique la siguiente corrección en la plantilla para revertir los nombres de las variables a los de la antigua versión de Qubes.

Puede editar la plantilla del siguiente modo:
...
## No se pudo instalar la plantilla en el dom0 a causa de una dependencia ausente (qubes-core-dom0-linux)
