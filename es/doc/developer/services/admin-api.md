...
lang: es
layout: doc-full
permalink: /es/doc/admin-api/
redirect_from:
- /es/doc/mgmt/
- /es/doc/mgmt1/
- /es/doc/mgmt-architecture/
- /es/doc/admin-api-architecture/
ref: 144
title: Admin API
translated: 'yes'
---
...
| `admin.vm.Create.<class>`              | `dom0`    | Plantilla  | `name=<name> label=<label>`               | -                                                         |
| `admin.vm.CreateInPool.<class>`        | `dom0`    | Plantilla  | `name=<name> label=<label> `<br/>`pool=<pool> pool:<volume>=<pool>`   | -                                                         | either use `pool=` to put all volumes there, <br/>or `pool:<volume>=` for individual volumes - both forms are not allowed at the same time
| `admin.vm.CreateDisposable`            | Plantilla  | -         | -                                         | name                                                      | Create new DisposableVM, `template` is any AppVM with `dispvm_allowed` set to True, or `dom0` to use default defined in `default_dispvm` property of calling VM; VM created with this call will be automatically removed after its shutdown; the main difference from `admin.vm.Create.DispVM` is automatic (random) name generation.
| `admin.vm.Remove`                      | vm        | -         | -                                         | -                                                         |
...
| `admin.vm.firewall.Get`                | vm        | -         | -                                         | `<rule>\n`                                                | rules syntax as in [firewall interface](/es/doc/vm-interface/#firewall-rules-in-4x) with addition of `expire=` and `comment=` options; `comment=` (if present) must be the last option
...
| `admin.vm.device.<class>.Attach`       | vm        | dispositivo    | options                                   | -                                                         | `device` is in form `<backend-name>+<device-ident>` <br/>optional options given in `key=value` format, separated with spaces; <br/>options can include `persistent=True` to "persistently" attach the device (default is temporary)
| `admin.vm.device.<class>.Detach`       | vm        | dispositivo    | -                                         | -                                                         | `device` is in form `<backend-name>+<device-ident>`
| `admin.vm.device.<class>.Set.persistent`| vm       | dispositivo    | `True`|`False`                            | -                                                         | `device` is in form `<backend-name>+<device-ident>`
...
| `admin.vm.volume.Info`                 | vm        | volumen    | -                                         | `<property>=<value>\n`                                    |
| `admin.vm.volume.Set.revisions_to_keep`| vm        | volumen    | value                                     | -                                                         |
| `admin.vm.volume.ListSnapshots`        | vm        | volumen    | -                                         | snapshot                                                  | duplicate of `admin.pool.volume.`, but with other call params |
| `admin.vm.volume.Snapshot`             | vm        | volumen    | -                                         | snapshot                                                  | id. |
| `admin.vm.volume.Revert`               | vm        | volumen    | snapshot                                  | -                                                         | id. |
| `admin.vm.volume.Resize`               | vm        | volumen    | size_in_bytes                             | -                                                         | id. |
| `admin.vm.volume.Import`               | vm        | volumen    | raw volume data                           | -                                                         | id. |
| `admin.vm.volume.CloneFrom`            | vm        | volumen    | -                                         | token, to be used in `admin.vm.volume.CloneTo`            | obtain a token to copy `volume` of `vm`;<br/>the token is one time use only, it's invalidated by `admin.vm.volume.CloneTo`, even if the operation fails |
| `admin.vm.volume.CloneTo`              | vm        | volumen    | token, obtained with `admin.vm.volume.CloneFrom` | -                                                         | copy volume pointed by a token to `volume` of `vm` |
| `admin.vm.Start`                       | vm        | -         | -                                         | -                                                         |
